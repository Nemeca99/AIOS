...
def demonstrate_ris_operator():
    ...
    for a, b, op in operations:
        ris_result = ris_meta_operator(a, b, op)
        result = ris_result['result']
        selected_op = ris_result['operation']
...
def demonstrate_magic_squares():
    ...
    is_valid, target_sum = validate_magic_square(magic_square)
    print(f"Standard 3x3 Magic Square is valid: {is_valid}, Magic sum: {target_sum}")
    
    is_valid, _ = validate_magic_square(non_magic)
    print(f"Non-magic Square is valid: {is_valid}")
