...
# Letter-to-number and number-to-letter mapping (A=1..Z=26, a=27..z=52)
...
def number_to_letter(n: int) -> str:
    """Convert number to letter using base-52 encoding (1-26=A-Z, 27-52=a-z)."""
    if n <= 0:
        return '0'
    if 1 <= n <= 26:
        ...
        # For numbers > 52, use a multi-letter encoding
        letters = []
        while n > 0:
            remainder = n % 52
            if remainder == 0:
                remainder = 52
            letters.insert(0, number_to_letter(remainder))
            n = (n - remainder) // 52
        return ''.join(letters)
