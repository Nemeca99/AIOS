...
def ris_meta_operator(
    a: Union[float, complex, str],
    b: Union[float, complex, str],
    operation: Optional[str] = None,
) -> Tuple[Union[float, complex, str], str]:
    """
...
def recursive_compress(value: Union[float, complex, str]) -> Union[float, complex, str]:
    ...
    if value > 0 and math.isclose(
        math.log(value), round(math.log(value)), rel_tol=1e-9
    ):
        log_val = math.log(value)
        return recursive_compress(log_val)
    # Prefer division by 10 for powers of 10
    if value > 10 and math.isclose(
        math.log10(value), round(math.log10(value)), rel_tol=1e-9
    ):
        div_val = value / 10
...
def parse_uml(expression: str) -> Dict:
    ...
    if expression.startswith("RIS(") and expression.endswith(")"):
        ...
        if len(operands) != 2:
            raise ValueError(
                f"RIS expects 2 operands, got {len(operands)}: {expression}"
            )
    ...
    if (
        ...
        return {
            "type": "multiplication",
            "operands": [parse_uml(op) for op in operands],
        }
    if expression.startswith("<>") and expression.endswith("<>"):
        ...
        if len(operands) != 2:
            raise ValueError(
                f"Division expects 2 operands, got {len(operands)}: {expression}"
            )
    ...
    if expression.startswith("@(") and expression.endswith(")"):
        ...
        if len(operands) != 2:
            raise ValueError(
                f"Power expects 2 operands, got {len(operands)}: {expression}"
            )
    ...
    if expression.startswith("!(") and expression.endswith(")"):
        ...
        if len(operands) != 2:
            raise ValueError(
                f"Complex number expects 2 operands, got {len(operands)}: {expression}"
            )
...
def eval_uml(parsed_expr: Dict) -> Union[float, complex, str]:
    ...
    if expr_type == "power":
        if len(operands) == 2:
            a, b = operands
            if isinstance(a, (int, float, complex)) and isinstance(
                b, (int, float, complex)
            ):
                return a**b
            else:
