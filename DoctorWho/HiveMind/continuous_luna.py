...
from hive_mind_logger import HiveMindLogger

class ContinuousLunaRunner:
    def __init__(self):
        ...
        self.logger.log("CONTINUOUS_LUNA", "Initializing Continuous Luna Runner...")
        self.luna = LunaMasterTest()
        self.logger.log("CONTINUOUS_LUNA", "✅ Continuous Luna Runner initialized")
        
    def signal_handler(self, signum, frame):
        """Handle shutdown signals gracefully"""
        self.logger.log("CONTINUOUS_LUNA", f"Received signal {signum}, initiating graceful shutdown...")
        self.running = False
        
    def run_continuous(self):
        """Run Luna continuously with random delays"""
        self.logger.log("CONTINUOUS_LUNA", "🌙 Starting Continuous Luna Data Gathering")
        self.logger.log("CONTINUOUS_LUNA", "============================================================")
        self.logger.log("CONTINUOUS_LUNA", f"⏰ Started: {self.start_time.strftime('%H:%M:%S')}")
        self.logger.log("CONTINUOUS_LUNA", "🔄 Mode: Continuous question processing")
        self.logger.log("CONTINUOUS_LUNA", "⏱️ Delay: 3-5 seconds between questions (random)")
        self.logger.log("CONTINUOUS_LUNA", "🛑 Press Ctrl+C to stop gracefully")
        self.logger.log("CONTINUOUS_LUNA", "============================================================")
        ...
        try:
            while self.running:
                ...
                self.logger.log("CONTINUOUS_LUNA", f"💤 Waiting {delay:.1f}s before next question...")
                time.sleep(delay)
                
        except KeyboardInterrupt:
            self.logger.log("CONTINUOUS_LUNA", "🛑 Keyboard interrupt received, shutting down...")
        except Exception as e:
            self.logger.log("CONTINUOUS_LUNA", f"❌ Error in continuous run: {e}", "ERROR")
        finally:
            self.shutdown()
            
    def ask_single_question(self):
        """Ask a single question and process the response"""
        ...
        self.logger.log("CONTINUOUS_LUNA", f"--- Q{self.question_count} ({question_data['trait']}) ---")
        self.logger.log("CONTINUOUS_LUNA", f"👤 User: {question_data['question']}")
        
        # Get Luna's response using the existing system
        start_time = time.time()
        response = self.get_luna_response(question_data['question'], question_data['trait'])
        response_time = time.time() - start_time
        
        self.logger.log("CONTINUOUS_LUNA", f"🌙 Luna: {response}")
        self.logger.log("CONTINUOUS_LUNA", f"⏱️ Time: {response_time:.1f}s")
        
        # Log some stats
        if self.question_count % 10 == 0:
            self.log_stats()
            
    def get_luna_response(self, question: str, trait: str) -> str:
        """Get Luna's response to a single question"""
        try:
            # Use the existing Luna system to generate a response
            # This is a simplified version that calls the internal methods
            response = self.luna._generate_response(question, trait)
            return response
        except Exception as e:
            self.logger.log("CONTINUOUS_LUNA", f"Error generating response: {e}", "ERROR")
    ...
    def log_stats(self):
        ...
        self.logger.log("CONTINUOUS_LUNA", f"📊 Stats: {self.question_count} questions, {runtime}")
        
    def shutdown(self):
        """Graceful shutdown"""
        runtime = datetime.now() - self.start_time
        self.logger.log("CONTINUOUS_LUNA", "🌙 Continuous Luna Data Gathering Complete!")
        self.logger.log("CONTINUOUS_LUNA", "============================================================")
        self.logger.log("CONTINUOUS_LUNA", f"⏰ Total Runtime: {runtime}")
        self.logger.log("CONTINUOUS_LUNA", f"❓ Questions Processed: {self.question_count}")
        self.logger.log("CONTINUOUS_LUNA", f"⚡ Avg Time per Question: {runtime.total_seconds() / max(self.question_count, 1):.1f}s")
        self.logger.log("CONTINUOUS_LUNA", "============================================================")

if __name__ == "__main__":
    runner = ContinuousLunaRunner()
