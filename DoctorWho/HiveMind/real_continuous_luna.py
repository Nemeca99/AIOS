#!/usr/bin/env python3
"""
Real Continuous Luna Runner
Uses the ACTUAL Luna system with real LLM calls for continuous data gathering
"""
...
class RealContinuousLunaRunner:
    def __init__(self):
        self.running = True
    ...
    def ask_single_question(self):
        ...
        # Use the REAL Luna system to get a response
        start_time = time.time()
        response = self.get_real_luna_response(question_data['question'], question_data['trait'])
    ...
    def get_real_luna_response(self, question: str, trait: str) -> str:
        """Get Luna's response using the REAL system with actual LLM calls"""
        try:
            # Create a temporary script that runs the real Luna system
            temp_script = f"""
import sys
import os
...
os.environ['PYTHONPATH'] = '/app' if 'PYTHONPATH' not in os.environ else os.environ['PYTHONPATH']
...
# Create a simple question format that Luna can process
question_text = "{question}"
trait = "{trait}"

# Use Luna's internal methods to get a real response
try:
    # This will use the actual LLM through Luna's system
    response = luna._process_single_question(question_text, trait)
    print(response)
except Exception as e:
    print(f"Error: {{e}}")
    # Fallback to a simple LLM call
    print("I'm processing your question...")
"""
            
            # Write the temp script
            with open("temp_luna_question.py", "w") as f:
                f.write(temp_script)
            
            # Run the script and capture output
            result = subprocess.run([sys.executable, "temp_luna_question.py"], 
                                  capture_output=True, text=True, timeout=30)
            
            # Clean up
            Path("temp_luna_question.py").unlink(missing_ok=True)
...
if __name__ == "__main__":
    runner = RealContinuousLunaRunner()
