...
class EnterpriseBilling:
    ...
    def get_usage(self, user_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # Aggregate metrics
            total_requests = sum(m.requests_count for m in user_metrics)
            total_fragments_stored = sum(m.fragments_stored for m in user_metrics)
            total_fragments_retrieved = sum(m.fragments_retrieved for m in user_metrics)
            total_search_queries = sum(m.search_queries for m in user_metrics)
            total_data_transferred = sum(m.data_transferred for m in user_metrics)
            
            # Calculate billing period
            oldest_metric = min(user_metrics, key=lambda m: m.start_time)
            billing_period = (datetime.now() - oldest_metric.start_time).days
            
            return {
                "user_id": user_id,
                "requests_count": total_requests,
                "fragments_stored": total_fragments_stored,
                "fragments_retrieved": total_fragments_retrieved,
                "search_queries": total_search_queries,
                "data_transferred": total_data_transferred,
                "billing_period_days": billing_period
            }
        except Exception as e:
            return {"error": f"Failed to get usage for {user_id}: {e}"}
...
class ComplianceManager:
    ...
    def get_audit_log(self, user_id: str = None, limit: int = 100) -> List[Dict[str, Any]]:
        """Get audit log entries for a specific user or all users"""
        try:
            if user_id:
                # Filter by user ID
            ...
            return [{"error": f"Failed to get audit log: {e}"}]
