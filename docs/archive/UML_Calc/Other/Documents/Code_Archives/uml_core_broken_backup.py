...
UML_OPERATORS = {
    ...
    'mul': {'symbol': '<', 'close': '>', 'func': lambda x: math.prod(x) if hasattr(math, 'prod') else eval('*'.join(map(str, x)))},
    'div': {'symbol': '<>', 'close': '<>', 'func': lambda x: x[0] / math.prod(x[1:]) if len(x) > 1 and hasattr(math, 'prod') else x[0]},
...
# For script execution (e.g., `python uml_core.py`)
