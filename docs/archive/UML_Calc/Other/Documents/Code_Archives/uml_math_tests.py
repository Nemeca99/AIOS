...
class UMLConformanceTests(unittest.TestCase):
    ...
    def test_advanced_operators(self):
        ...
        test_cases = [
            ...
            ("√25", 5),        # Square root (if supported)
            ("1/8", 0.125),    # Reciprocal
        ]
        ...
        for uml_expr, expected in test_cases:
            try:
                ...
                print(f"Skipping test for {uml_expr}: {str(e)}")
    ...
    def test_complex_numbers(self):
        ...
        # These tests will be skipped if complex numbers are not supported
        try:
            # Simple tests for complex numbers
            uml_expr = "[0,[0,i,1]]"  # 0 + 1i
            parsed_uml = parse_uml(uml_expr)
            uml_result = eval_uml(parsed_uml)
            self.assertEqual(complex(0, 1), uml_result)
            
            # More complex tests
            uml_expr = "<[1,[0,i,2]],[3,[0,i,4]]>"  # (1+2i) * (3+4i)
            parsed_uml = parse_uml(uml_expr)
            uml_result = eval_uml(parsed_uml)
