...
# Add parent directory to path for AIOS imports
...
class DreamQuickNapMiddleware:
            ...
            if hasattr(self, 'meditation_responses') and self.meditation_responses:
                for response in self.meditation_responses[-5:]:  # Store last 5 responses
                    if response and len(response.strip()) > 10:  # Only store substantial responses
                        # Generate unique fragment ID with timestamp and random component
                        import time
                        import uuid
    ...
    def _perform_meditation_phase(self, phase_number: int) -> Dict:
        ...
        try:
            # Generate varied meditation questions for deeper self-reflection
            questions = [
            ...
            # Select question based on phase number for variety
            question_index = (phase_number - 1) % len(questions)
...
"""
    ...
    def run_quick_nap(self, duration_minutes: int = 30, dream_cycles: int = 2, meditation_blocks: int = 1, verbose: bool = False):
        """Public method to run quick nap with configurable parameters."""
        self.total_runtime_minutes = duration_minutes
        self.rem_cycles_per_phase = dream_cycles
        self.meditation_blocks_per_phase = meditation_blocks
