...
try:
    from luna_master_test_clean import LunaMasterTest
    ...
    from test_carma_full_evolution import test_full_evolution
...
def run_luna_test(questions=None):
    ...
    try:
        luna = LunaMasterTest()
        results = luna.run_master_test(questions, "consciousness_test")
        
        print(f"\n📊 Luna Test Results:")
        print(f"   Questions: {results['statistics']['total_questions']}")
        print(f"   Avg response time: {results['statistics']['avg_response_time']:.2f}s")
        print(f"   Avg quality score: {results['statistics']['avg_quality_score']:.3f}")
...
def main():
    ...
    if args.command == "test":
        ...
        success = run_consciousness_demo()
        return 0 if success else 1
        ...
        if args.questions:
            try:
                ...
                return 1
        success = run_luna_test(questions)
        return 0 if success else 1
    
    elif args.command == "analyze":
        success = run_cache_analysis()
        return 0 if success else 1
    
    elif args.command == "autonomous":
        success = run_autonomous_mode(args.duration)
        return 0 if success else 1
